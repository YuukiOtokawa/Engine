...
private:
	...
	// ProcessMessage関数用関数 気にしなくてよし
	void KeyUp(int key);
	void KeyDown(int key);

	/// @brief コンストラクタ 各種ステートの初期化
	Keyboard() { Reset(); }

	Keyboard(const Keyboard&) = delete; // コピーコンストラクタ禁止
	Keyboard& operator=(const Keyboard&) = delete; // 代入演算子禁止

	Keyboard(Keyboard&&) = delete; // ムーブコンストラクタ禁止
	Keyboard& operator=(Keyboard&&) = delete; // ムーブ代入演算子禁止

	static Keyboard* m_pInstance; // インスタンス
public:
	...
	/// @brief 各種ステートの初期化 コンストラクタ以外で使うことないよね？
	void Reset();
	...
	/// @brief キーを離した瞬間を検知
	/// @param key キーコード Keyboard_Keys列挙体を使用
	/// @return 
