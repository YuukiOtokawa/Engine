...
public:
	...
	/// @brief オブジェクトまたはコンポーネントの状態を更新します。
	void Update();
	/// @brief 描画処理を実行します。
	void Draw();
	/// @brief リソースの解放や後処理を行います。
	void Finalize();

	/// @brief オブジェクトにコンポーネントを追加します。
	/// @tparam T 追加するコンポーネントの型。TはComponentクラスを継承している必要があります。
	/// @tparam ...Args 
	/// @param ...args 
	...
	/// @brief 指定された型の最初のコンポーネントへのポインタを取得します。
	/// @tparam T 取得したいコンポーネントの型。
	/// @return 指定された型に一致する最初のコンポーネントへのポインタ。一致するコンポーネントが存在しない場合はnullptrを返します。
	template<typename T>
	T* GetComponent() {
		for (Component* component : m_Components) {
	...
	/// @param tag 設定するGameObjectTag値。
	void SetTag(GameObjectTag tag) {
		m_Tag = tag;
	}

	/// @brief オブジェクトのタグを取得します。
	/// @return このオブジェクトに関連付けられている GameObjectTag を返します。
