...
{
    // モザイクのブロックサイズ（小さいほど細かいモザイク）
    float2 mosaicSize = float2(32.0f, 32.0f);
    ...
    // 中心から一定距離内でモザイク効果を適用
    if (distance < 0.3f) // 0.15fから0.3fに変更してより広い範囲に適用
    {
        // UVを画面解像度に変換
        float2 pixelCoord = uv * mosaicSize;
