...
void OBJLoader::Load(const char* FileName, Object* object)
{
	...
	*material = *m_Model->MaterialArray;
	...
	object->GetComponent<MeshFilter>()->SetVertexInfo(m_Model->pVertexIndex);
...
MODEL_OBJ OBJLoader::LoadModel(const char* FileName, MODEL* Model)
{
	...
	vi->SetVertexInfo(std::vector<VERTEX>(modelObj.VertexArray, modelObj.VertexArray + modelObj.VertexNum));
	vi->SetIndexInfo(std::vector<unsigned int>(modelObj.IndexArray, modelObj.IndexArray + modelObj.IndexNum));
	vi->SetName(PathFindFileNameA(FileName));
	vi->SetFilePath(FileName);
	Model->pVertexIndex = vi;
	...
	while (true)
	{
		...
		else if (strcmp(str, "f") == 0)
		{
			...
			do
			{
				...
				ModelObj->VertexArray[vc].color = Vector4O(1.0f, 1.0f, 1.0f, 1.0f);
