...
public:
    ...
    static std::chrono::duration<double, std::micro> deltaTime; // 前フレームからの経過時間（ミリ秒）
    static std::chrono::duration<double, std::micro> elapsedTime; // 起動からの累積時間（ミリ秒）
    ...
    static double DeltaTime() { return deltaTime.count(); }
    static double ElapsedTime() { return elapsedTime.count(); }
