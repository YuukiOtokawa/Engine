							...
							const FileSystemUtils::Path&				moduleName_ )
{
	if( m_pImplData->m_bFindVS )
	{
		...
		else
		{
			...
			{
				EngineConsole::Log("No Supported Compiler for RCC++ found.\n", LogLevel::Error);
    ...
    {
		EngineConsole::Log("No Supported Compiler for RCC++ found, cannot compile changes.\n", LogLevel::Error);
...
// Add improved debugging options if available: http://randomascii.wordpress.com/2013/09/11/debugging-optimized-codenew-in-visual-studio-2012/
	...
	if( NULL == m_pImplData->m_CmdProcess.m_CmdProcessInfo.hProcess )
	{
        ...
        m_pImplData->m_CmdProcess.WriteInput(chcp65001); // set utf-8 console locale
	}
	...
	if ( !compilerOptions_.intermediatePath.Exists() )
	{
		...
		if( success && m_pImplData->m_pLogger ) { 
			EngineConsole::Log("Created intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str());
			m_pImplData->m_pLogger->LogInfo("Created intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str());
		}
		else if( m_pImplData->m_pLogger ) { 
			EngineConsole::LogError("Error creating intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str());
			m_pImplData->m_pLogger->LogError("Error creating intermediate folder \"%s\"\n", compilerOptions_.intermediatePath.c_str()); 
		}
	}
	...
	if (m_pImplData->m_pLogger) {
		EngineConsole::Log(("%s", clCommandOptions.c_str()));
		m_pImplData->m_pLogger->LogInfo("%s", clCommandOptions.c_str()); // use %s to prevent any tokens in compile string being interpreted as formating
	}
    ...
    {
		if (m_pImplData->m_pLogger) {
			EngineConsole::Log(("Could not create CL Command File %s\n  - Falling back to command line options\n", pathTempCLCommandFile.c_str()));
			m_pImplData->m_pLogger->LogInfo("Could not create CL Command File %s\n  - Falling back to command line options\n", pathTempCLCommandFile.c_str());
		}// use %s to prevent any tokens in compile string being interpreted as formating
...
void GetPathsOfVisualStudioInstalls( std::vector<VSVersionInfo>* pVersions, ICompilerLogger* pLogger )
{
	...
	default:
		...
		{
			char logBuffer[512];
			snprintf(logBuffer, sizeof(logBuffer), "WARNING: VS Compiler with _MSC_VER %d potentially not supported. Defaulting to version %s.\n", MSCVERSION, VS_DISCOVERY_INFO[startVersion].versionName);
	...
	while( bReadActive )
	{
										...
										&nBytesRead,NULL) || !nBytesRead)
		{
			...
			if( GetLastError() != ERROR_BROKEN_PIPE)	//broken pipe is OK
			{
				if (pCmdProc->m_pLogger) {
					EngineConsole::Log("[RuntimeCompiler] Redirect of compile output failed on read\n");
					pCmdProc->m_pLogger->LogError("[RuntimeCompiler] Redirect of compile output failed on read\n");
				}
			}
		}
		else
		{
			...
			if( found != std::string::npos )
			{
				...
				if (!pCmdProc->m_bStoreCmdOutput && pCmdProc->m_pLogger) {
					EngineConsole::Log("[RuntimeCompiler] Complete\n");
					pCmdProc->m_pLogger->LogInfo("[RuntimeCompiler] Complete\n"); 
				}
			...
			if( bReadActive || buffer.length() ) //don't output blank last line
			{
				...
				else
				{
					...
					{
						if (pCmdProc->m_pLogger) { 
							EngineConsole::Log(("%s", buffer.c_str()), LogLevel::Error);
							pCmdProc->m_pLogger->LogError("%s", buffer.c_str());
						}
					}
					else
					{
						if (pCmdProc->m_pLogger) {
							EngineConsole::Log(("%s", buffer.c_str()));
							pCmdProc->m_pLogger->LogInfo("%s", buffer.c_str());
						}
					}
				}
			}
...
void CmdProcess::InitialiseProcess()
{
	...
	{
		EngineConsole::Log("[RuntimeCompiler] Failed to create output redirection pipe\n", LogLevel::Error);
	...
	{
		EngineConsole::Log("[RuntimeCompiler] Failed to duplicate error output redirection pipe\n", LogLevel::Error);
	...
	if (si.hStdOutput)
	{
		...
		{
			EngineConsole::Log("[RuntimeCompiler] Failed to duplicate output read pipe\n", LogLevel::Error);
	...
	{
		EngineConsole::Log("[RuntimeCompiler] Failed to create input pipes\n", LogLevel::Error);
	...
	if (si.hStdOutput)
	{
		if (!DuplicateHandle(GetCurrentProcess(), hInputWriteTmp,
			...
			DUPLICATE_SAME_ACCESS))
		{
			EngineConsole::Log("[RuntimeCompiler] Failed to duplicate input write pipe\n", LogLevel::Error);
