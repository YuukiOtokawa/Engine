...
#include "../../../EngineSystem/EngineConsole.h"
...
void ObjectFactorySystem::ProtectedObjectSwapper::ProtectedFunc()
{
	...
	// serialize all out
	EngineConsole::Log("Serializing out from %d old constructors...\n", (int)m_ConstructorsOld.size());
	if (m_pLogger) {

		m_pLogger->LogInfo( "Serializing out from %d old constructors...\n", (int)m_ConstructorsOld.size());
	}
	...
	// swap serializer
	EngineConsole::Log("Swapping in and creating objects for %d new constructors...\n", (int)m_ConstructorsToAdd.size());
	...
	EngineConsole::Log("Serialising in...\n");
	...
	EngineConsole::Log("Auto Constructing Singletons...\n");
    ...
    {
		EngineConsole::Log("Initialising and testing new serialisation...\n");
	    if( m_pLogger ) m_pLogger->LogInfo( "Initialising and testing new serialisation...\n");
    }
    else
    {
		EngineConsole::Log("Initialising...\n");
...
bool ObjectFactorySystem::HandleRedoUndo( const TConstructors& constructors )
{
	...
	{
		EngineConsole::Log("ObjectFactorySystem::HandleRedoUndo() called with no constructors.\n");
...
void ObjectFactorySystem::AddConstructors( IAUDynArray<IObjectConstructor*> &constructors )
{
	if( constructors.Size() == 0 )
	{
		EngineConsole::Log("ObjectFactorySystem::AddConstructors() called with no constructors.\n");
	...
	{
		EngineConsole::Log("Need to fast forward undo system to current state of source code.\n");
...
void ObjectFactorySystem::CompleteConstructorSwap( ProtectedObjectSwapper& swapper )
{
	if( swapper.HasHadException() && PHASE_DELETEOLD != swapper.m_ProtectedPhase )
	{
		if( m_pLogger )
		{
			EngineConsole::Log("Exception during object swapping, switching back to previous objects.\n", LogLevel::Error);
	...
	{
		EngineConsole::Log("Object swap completed\n");
		if( m_pLogger ) m_pLogger->LogInfo( "Object swap completed\n");
		if( swapper.HasHadException() && PHASE_DELETEOLD == swapper.m_ProtectedPhase )
		{
			EngineConsole::Log("Exception during object destruction of old objects, leaking.\n", LogLevel::Error);
