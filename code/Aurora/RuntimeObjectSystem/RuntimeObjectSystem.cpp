...
void RuntimeObjectSystem::OnFileChange(const IAUDynArray<const char*>& filelist)
{
    ...
    for( unsigned short proj = 0; proj < m_Projects.size(); ++proj )
    {
        ...
        EngineConsole::Log("FileChangeNotifier triggered recompile with changes to:\n");
...
void RuntimeObjectSystem::StartRecompile()
{
    ...
    {
        EngineConsole::Log("Error - could not find any files to build in any projects\n", LogLevel::Error);
    ...
    m_Projects[project].m_CompilerOptions.compileOptions = "/std:c++20 " + m_Projects[project].m_CompilerOptions.compileOptions;
    //m_Projects[project].m_CompilerOptions.linkOptions = " OtokawaEngine.lib";
...
bool RuntimeObjectSystem::LoadCompiledModule()
{
	...
	if (!module)
	{
        EngineConsole::LogError("Failed to load module %s\n", m_CurrentlyCompilingModuleName.c_str());
	...
	{
        EngineConsole::Log("Failed GetProcAddress\n", LogLevel::Error);
    ...
    EngineConsole::Log("Compilation Succeeded\n");
...
void RuntimeObjectSystem::SetupRuntimeFileTracking(const IAUDynArray<IObjectConstructor*>& constructors_)
{
	...
	for (size_t i = 0, iMax = constructors_.Size(); i < iMax; ++i)
	{
        ...
        {
        #ifdef _WIN32
            EngineConsole::Log("Source file not found may be due to compile option /FC and /utf-8 not being set.\n", LogLevel::Warning);
        ...
        #else
            EngineConsole::Log("Source file not found may be due to macro COMPILE_PATH not being set.\n", LogLevel::Warning);
...
FileSystemUtils::Path RuntimeObjectSystem::FindFile( const FileSystemUtils::Path& input, bool* pFound )
{
                ...
                if( depth )
                {
                    ...
                    if( directory.Exists() )
                    {
                        ...
                        if( foundFile.Exists() )
                        {
                            ...
                            char logBuffer[512];
                            snprintf(logBuffer, sizeof(logBuffer), "Found Directory Mapping: %s to %s\n", requestedDirectory.c_str(), directory.c_str());
    ...
    if( !bFound )
    {
        EngineConsole::Log("Could not find Directory Mapping for: %s\n", input.c_str(), LogLevel::Warning);
...
bool RuntimeObjectSystem::TestBuildCallback(const char* file, TestBuildResult type)
{
    ...
    case TESTBUILDRRESULT_SUCCESS:            // SUCCESS, yay!
        EngineConsole::Log("TESTBUILDRRESULT_SUCCESS: %s\n", file);
    ...
    case TESTBUILDRRESULT_NO_FILES_TO_BUILD:  // file registration error or no runtime files of this type
        EngineConsole::LogWarning("TESTBUILDRRESULT_NO_FILES_TO_BUILD\n");
    ...
    case TESTBUILDRRESULT_BUILD_FILE_GONE:    // the file is no longer present
        EngineConsole::LogError("TESTBUILDRRESULT_BUILD_FILE_GONE: %s\n", file);
    ...
    case TESTBUILDRRESULT_BUILD_NOT_STARTED:  // file change detection could be broken, or if an include may not be included anywhere
        EngineConsole::LogError("TESTBUILDRRESULT_BUILD_NOT_STARTED: %s\n", file);
    ...
    case TESTBUILDRRESULT_BUILD_FAILED:       // a build was started, but it failed or module failed to load. See log.
        EngineConsole::LogError("TESTBUILDRRESULT_BUILD_FAILED: %s\n", file);
    ...
    case TESTBUILDRRESULT_OBJECT_SWAP_FAIL:   // build succeeded, module loaded but errors on swapping
        EngineConsole::LogError("TESTBUILDRRESULT_OBJECT_SWAP_FAIL: %s\n", file);
                          ...
                          ITestBuildNotifier* callback, bool bTestFileTracking )
{
    ...
    EngineConsole::Log("Testing change to file: %s\n", file.c_str());
...
{
    EngineConsole::Log("TestBuildAllRuntimeSourceFiles Starting\n");
    ...
    if( 0 == numErrors )
    {
        EngineConsole::Log("All Tests Passed\n");
        if( m_pCompilerLogger ) { m_pCompilerLogger->LogInfo("All Tests Passed\n"); }
    }
    else
    {
        EngineConsole::LogError("Tests Failed: %d\n", numErrors);
