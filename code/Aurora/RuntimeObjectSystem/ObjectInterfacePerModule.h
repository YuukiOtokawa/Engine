...
public:
	...
	virtual IObject* Construct(bool editable)
	{
	...
	virtual IObject* GetConstructedObject( PerTypeObjectId id ) const
	{
		...
		{
            auto obj = dynamic_cast<IObject*>(m_ConstructedObjects[id]);
            return obj;
		}
		return 0;
	}
...
private:
...
#ifndef RCCPPOFF
	// REGISTERBASE マクロ - ランタイム再コンパイル機能付きでクラスを登録
	// T: 登録するクラス型
	// bIsSingleton: シングルトンかどうか（true=シングルトン、false=通常クラス）  
	// bIsAutoConstructSingleton: シングルトンを自動生成するかどうか
	#define REGISTERBASE( T, bIsSingleton, bIsAutoConstructSingleton )	\
	/* ランタイム追跡用の静的オブジェクトを生成。__COUNTER__で一意のIDを割り当て */ \
	static RuntimeTracking< __COUNTER__ >	   g_runtimeTrackingList_##T; \
	/* TActualクラスのコンストラクタを特殊化。ファイル名とランタイム追跡情報を渡す */ \
	...
	/* クラス名を文字列として返す静的関数を特殊化 */ \
	template<> const char* TActual< T >::GetTypeNameStatic() { return #T; } \
	/* TActualテンプレートクラスを明示的にインスタンス化 */ \
	template class TActual< T >;
#else
	// REGISTERBASE マクロ - ランタイム再コンパイル機能なしでクラスを登録
	// RCCPPOFF定義時は追跡機能を無効化してシンプルな登録のみ行う
	#define REGISTERBASE( T, bIsSingleton, bIsAutoConstructSingleton )	\
	/* コンストラクタの特殊化（ファイル追跡なし） */ \
...
//注意: ファイルマクロは、Visual Studioで /FC オプションまたは /ZI オプション（/FCを強制する）を使用した場合のみフルパスを出力します
